...
self.onmessage = function (e) {
  ...
  if (data.init) {
    ...
    renderer.setSize(width, height); // TODO fix passing maxRecursionDepth as parameter.
...
THREE.RaytracingRendererWorker = function () {
  ...
  var spawnRay = function () {
    ...
    return function spawnRay(rayOrigin, rayDirection, outputColor, recursionDepth) {
      ...
      var intersections = raycaster.intersectObjects(objects, true); // ray didn't find anything
      // (here should come setting of background color?)

      if (intersections.length === 0) return; // ray hit
      ...
      eyeVector.subVectors(ray.origin, point).normalize(); // resolve pixel diffuse color

      if (material.isMeshLambertMaterial || material.isMeshPhongMaterial || material.isMeshBasicMaterial) {
      ...
      if (material.isMeshBasicMaterial) {
        for (var i = 0, l = lights.length; i < l; i++) {
          ...
          var intersections = raycasterLight.intersectObjects(objects, true); // point in shadow

          if (intersections.length > 0) continue; // point visible
        ...
        for (var i = 0, l = lights.length; i < l; i++) {
          ...
          if (intersections.length > 0) continue; // point lit
          ...
          lightColor.copyGammaToLinear(light.color); // compute attenuation
          ...
          lightVector.normalize(); // compute diffuse
          ...
          outputColor.add(lightContribution); // compute specular
  ...
  var computePixelNormal = function () {
    ...
    return function computePixelNormal(outputVector, point, flatShading, face, geometry) {
      ...
      if (flatShading === true) {
        ...
        vC.fromBufferAttribute(positions, face.c); // compute barycentric coordinates
        ...
        var c = 1.0 - a - b; // compute interpolated vertex normal
    ...
    return function renderBlock(blockX, blockY) {
      ...
      for (var y = 0; y < blockSize; y++) {
        for (var x = 0; x < blockSize; x++, index += 4) {
          ...
          spawnRay(origin, direction, pixelColor, 0); // convert from linear to gamma
  ...
  this.render = function (scene, camera) {
    ...
    objects = scene.children; // collect lights and set up object matrices
