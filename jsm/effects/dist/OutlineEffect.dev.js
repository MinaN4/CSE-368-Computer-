...
var OutlineEffect = function OutlineEffect(renderer, parameters) {
  ...
  var defaultColor = new _threeModule.Color().fromArray(parameters.defaultColor !== undefined ? parameters.defaultColor : [0, 0, 0]);
  ...
  var fragmentShader = ["#include <common>", "#include <fog_pars_fragment>", "#include <logdepthbuf_pars_fragment>", "#include <clipping_planes_pars_fragment>", "uniform vec3 outlineColor;", "uniform float outlineAlpha;", "void main() {", "	#include <clipping_planes_fragment>", "	#include <logdepthbuf_fragment>", "	gl_FragColor = vec4( outlineColor, outlineAlpha );", "	#include <premultiplied_alpha_fragment>", "	#include <tonemapping_fragment>", "	#include <encodings_fragment>", "	#include <fog_fragment>", "}"].join("\n");

  function createMaterial() {
    return new _threeModule.ShaderMaterial({
      type: 'OutlineEffect',
      uniforms: _threeModule.UniformsUtils.merge([_threeModule.UniformsLib['fog'], _threeModule.UniformsLib['displacementmap'], uniformsOutline]),
