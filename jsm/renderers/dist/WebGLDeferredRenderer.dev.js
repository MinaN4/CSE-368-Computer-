...
exports.ShaderDeferred = exports.ShaderDeferredCommon = exports.DeferredShaderChunk = exports.WebGLDeferredRenderer = void 0;

var _threeModule = require("../../../build/three.module.js");

var _EffectComposer = require("../postprocessing/EffectComposer.js");

var _ShaderPass = require("../postprocessing/ShaderPass.js");

var _RenderPass = require("../postprocessing/RenderPass.js");

var _FXAAShader = require("../shaders/FXAAShader.js");
 ...
 *  - addEffect
 */
var WebGLDeferredRenderer = function WebGLDeferredRenderer(parameters) {
  parameters = parameters || {}; // private properties
  ...
  var _width, _height; // for Classic Deferred Rendering


  var _compColor;

  var _passColor, _passForward, _passCopy; // for Light Pre-Pass


  var _compReconstruction;

  var _passReconstruction; // for Common
  ...
  var _tmpMaterial = new _threeModule.ShaderMaterial({
    visible: false
  });

  var _tmpVector3 = new _threeModule.Vector3(); // scene/material/light cache for deferred rendering.
  ...
  var _lightFullscreenScenesCache = {}; // object.material.uuid -> deferredMaterial or
  ...
  var _reconstructionMaterialsCache = {}; // originalLight.uuid -> deferredLight

  var _deferredLightsCache = {}; // deferredLight.uuid -> deferredLightMaterial

  var _classicDeferredLightMaterialsCache = {};
  var _lightPrePassMaterialsCache = {};
  var _removeThresholdCount = 60; // deferredMaterials.uuid -> object.material or
  // deferredMaterials.uuid -> object.material[ n ]
  // save before render and release after render.

  var _originalMaterialsTable = {}; // object.uuid -> originalOnBeforeRender
  // save before render and release after render.

  var _originalOnBeforeRendersTable = {}; // object.material.uuid -> object.material.visible or
  // object.material[ i ].uuid -> object.material[ i ].visible or
  // save before render and release after render.

  var _originalVisibleTable = {}; // external properties
  ...
  function init(parameters) {
    _this.renderer = parameters.renderer !== undefined ? parameters.renderer : new _threeModule.WebGLRenderer();
    _this.domElement = _this.renderer.domElement;
    _context = _this.renderer.getContext();
    _state = _this.renderer.state;
    _width = parameters.width !== undefined ? parameters.width : _this.renderer.getSize(new _threeModule.Vector2()).width;
    _height = parameters.height !== undefined ? parameters.height : _this.renderer.getSize(new _threeModule.Vector2()).height;
  ...
  function initDepthTexture() {
    _depthTexture = new _threeModule.DepthTexture(_width, _height, _threeModule.UnsignedInt248Type, undefined, undefined, undefined, undefined, undefined, undefined, _threeModule.DepthStencilFormat);
  }

  function initPassNormalDepth() {
    _passNormalDepth = new _RenderPass.RenderPass();
    _passNormalDepth.clear = true;
    var rt = new _threeModule.WebGLRenderTarget(_width, _height, {
    ...
    _compNormalDepth = new _EffectComposer.EffectComposer(_this.renderer, rt);
  ...
  function initPassColor() {
    _passColor = new _RenderPass.RenderPass();
    ...
    rt.texture.generateMipamps = false;
    _compColor = new _EffectComposer.EffectComposer(_this.renderer, rt);
  ...
  function initPassLight() {
    _passLightFullscreen = new _RenderPass.RenderPass();
    _passLightFullscreen.clear = true;
    _passLightFullscreen.camera = new _threeModule.OrthographicCamera(-1, 1, 1, -1, 0, 1);
    _passLight = new _RenderPass.RenderPass();
    ...
    rt.texture.generateMipamps = false;
    _compLight = new _EffectComposer.EffectComposer(_this.renderer, rt);
  ...
  function initPassReconstruction() {
    _passReconstruction = new _RenderPass.RenderPass();
    ...
    rt.texture.generateMipamps = false;
    _compReconstruction = new _EffectComposer.EffectComposer(_this.renderer, rt);
  ...
  function initPassFinal() {
    _passFinal = new _ShaderPass.ShaderPass(ShaderDeferred['final']);
    _passFinal.clear = true;
    _passFinal.uniforms.samplerResult.value = _compLight.renderTarget2.texture;
    _passFinal.material.blending = _threeModule.NoBlending;
    _passFinal.material.depthWrite = false;
    _passFinal.material.depthTest = false;
    _passForward = new _RenderPass.RenderPass();
    _passForward.clear = false;
    _passCopy = new _ShaderPass.ShaderPass(_CopyShader.CopyShader);
    _passFXAA = new _ShaderPass.ShaderPass(_FXAAShader.FXAAShader);
    var rt = new _threeModule.WebGLRenderTarget(_width, _height, {
      minFilter: _threeModule.NearestFilter,
    ...
    rt.texture.generateMipamps = false;
    _compFinal = new _EffectComposer.EffectComposer(_this.renderer, rt);
  ...
  function initLightScene(scene) {
    ...
    var lightFullscreenScene = lightFullscreenSceneData.scene; // emissiveLight is only for Classic Deferred Rendering
  ...
  function getNormalDepthMaterial(originalMaterial) {
    return getMaterialFromCacheOrCreate(originalMaterial, _lightPrePass ? _normalDepthShininessMaterialsCache : _normalDepthMaterialsCache, createDeferredNormalDepthMaterial, updateDeferredNormalDepthMaterial);
  }
  ...
  function getColorMaterial(originalMaterial) {
    return getMaterialFromCacheOrCreate(originalMaterial, _colorMaterialsCache, createDeferredColorMaterial, updateDeferredColorMaterial);
  }
  ...
  function getReconstructionMaterial(originalMaterial) {
    ...
    return getMaterialFromCacheOrCreate(originalMaterial, _reconstructionMaterialsCache, createDeferredReconstructionMaterial, updateDeferredReconstructionMaterial);
  }
  ...
  function createDeferredLightMesh(light, geometry) {
    var mesh = new _threeModule.Mesh(geometry, _tmpMaterial);
  ...
  function createDeferredPointLight(light) {
    var mesh = createDeferredLightMesh(light, new _threeModule.SphereBufferGeometry(1, 16, 8));
  ...
  function createDeferredPointLightMaterial() {
    ...
    material.side = _threeModule.BackSide;
    material.depthFunc = _threeModule.GreaterEqualDepth;
    return material;
  }
  ...
  function createDeferredSpotLight(light) {
    var mesh = createDeferredLightMesh(light, new _threeModule.PlaneBufferGeometry(2, 2));
  ...
  function saveOriginalOnBeforeRenderAndCheckTransparency(object) {
    if (object.material === undefined) return;
    _originalOnBeforeRendersTable[object.uuid] = object.onBeforeRender; // _hasTransparentObject is used only for Classic Deferred Rendering
...
var DeferredShaderChunk = {
  packVector3: ["float vec3_to_float( vec3 data ) {", "	const float unit = 255.0/256.0;", "	highp float compressed = fract( data.x * unit ) + floor( data.y * unit * 255.0 ) + floor( data.z * unit * 255.0 ) * 255.0;", "	return compressed;", "}"].join("\n"),
...
var ShaderDeferred = {
  ...
  color: {
    uniforms: {
      map: new _threeModule.Uniform(null),
      offsetRepeat: new _threeModule.Uniform(new _threeModule.Vector4(0, 0, 1, 1)),
      diffuse: new _threeModule.Uniform(new _threeModule.Color(0x000000)),
      emissive: new _threeModule.Uniform(new _threeModule.Color(0x000000)),
      specular: new _threeModule.Uniform(new _threeModule.Color(0x000000)),
      shininess: new _threeModule.Uniform(30.0)
    },
    vertexShader: ["#include <uv_pars_vertex>", "#include <morphtarget_pars_vertex>", "#include <skinning_pars_vertex>", "void main() {", "#include <uv_vertex>", "#include <begin_vertex>", "#include <beginnormal_vertex>", "#include <skinbase_vertex>", "#include <skinnormal_vertex>", "#include <defaultnormal_vertex>", "#include <morphtarget_vertex>", "#include <skinning_vertex>", "#include <project_vertex>", "}"].join("\n"),
  ...
  emissiveLight: {
    uniforms: Object.assign({
      samplerColor: new _threeModule.Uniform(null)
  ...
  pointLight: {
    uniforms: Object.assign({
      samplerNormalDepth: new _threeModule.Uniform(null),
      samplerColor: new _threeModule.Uniform(null),
      lightColor: new _threeModule.Uniform(new _threeModule.Color(0x000000)),
      lightPositionVS: new _threeModule.Uniform(new _threeModule.Vector3(0, 1, 0)),
      lightIntensity: new _threeModule.Uniform(1.0),
      lightRadius: new _threeModule.Uniform(1.0)
    }, ShaderDeferredCommon['commonUniforms']),
  ...
  spotLight: {
    uniforms: Object.assign({
      ...
      samplerColor: new _threeModule.Uniform(null),
      lightColor: new _threeModule.Uniform(new _threeModule.Color(0x000000)),
      lightDirectionVS: new _threeModule.Uniform(new _threeModule.Vector3(0, 1, 0)),
      lightPositionVS: new _threeModule.Uniform(new _threeModule.Vector3(0, 1, 0)),
      lightAngle: new _threeModule.Uniform(1.0),
      lightIntensity: new _threeModule.Uniform(1.0)
    }, ShaderDeferredCommon['commonUniforms']),
    vertexShader: ["void main() { ", "	gl_Position = vec4( sign( position.xy ), 0.0, 1.0 );", "}"].join("\n"),
  ...
  pointLightPre: {
    uniforms: Object.assign({
      samplerNormalDepthShininess: new _threeModule.Uniform(null),
      lightColor: new _threeModule.Uniform(new _threeModule.Color(0x000000)),
      lightPositionVS: new _threeModule.Uniform(new _threeModule.Vector3(0, 1, 0)),
      lightIntensity: new _threeModule.Uniform(1.0),
      lightRadius: new _threeModule.Uniform(1.0)
  ...
  reconstruction: {
    uniforms: Object.assign({
      samplerLight: new _threeModule.Uniform(null),
      map: new _threeModule.Uniform(null),
      offsetRepeat: new _threeModule.Uniform(new _threeModule.Vector4(0, 0, 1, 1)),
      diffuse: new _threeModule.Uniform(new _threeModule.Color(0x000000)),
      emissive: new _threeModule.Uniform(new _threeModule.Color(0x000000)),
  ...
  "final": {
    uniforms: {
      samplerResult: new _threeModule.Uniform(null)
    },
...
exports.ShaderDeferred = ShaderDeferred;