...
var ShadowMapViewer = function ShadowMapViewer(light) {
  ...
  var camera = new _threeModule.OrthographicCamera(window.innerWidth / -2, window.innerWidth / 2, window.innerHeight / 2, window.innerHeight / -2, 1, 10);
  camera.position.set(0, 0, 2);
  var scene = new _threeModule.Scene(); //HUD for shadow map
  ...
  var plane = new _threeModule.PlaneBufferGeometry(frame.width, frame.height);
  var mesh = new _threeModule.Mesh(plane, material);
  ...
  if (doRenderLabel) {
    ...
    var labelTexture = new _threeModule.Texture(labelCanvas);
    labelTexture.magFilter = _threeModule.LinearFilter;
    labelTexture.minFilter = _threeModule.LinearFilter;
    labelTexture.needsUpdate = true;
    var labelMaterial = new _threeModule.MeshBasicMaterial({
      map: labelTexture,
      side: _threeModule.DoubleSide
    });
    labelMaterial.transparent = true;
    var labelPlane = new _threeModule.PlaneBufferGeometry(labelCanvas.width, labelCanvas.height);
    labelMesh = new _threeModule.Mesh(labelPlane, labelMaterial);
    scene.add(labelMesh);
  }
