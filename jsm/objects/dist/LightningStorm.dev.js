...
var LightningStorm = function LightningStorm(stormParams) {
  _threeModule.Object3D.call(this); // Parameters
  ...
  this.lightningParameters = _LightningStrike.LightningStrike.copyParameters(stormParams.lightningParameters, stormParams.lightningParameters);
  this.lightningParameters.isEternal = false;
  this.lightningMaterial = stormParams.lightningMaterial !== undefined ? stormParams.lightningMaterial : new _threeModule.MeshBasicMaterial({
    color: 0xB0FFFF
  });
  ...
  if (stormParams.onRayPosition !== undefined) {
    ...
    this.onRayPosition = function (source, dest) {
      ...
      var height = _threeModule.Math.lerp(stormParams.minHeight, stormParams.maxHeight, Math.random());
  ...
  for (var i = 0; i < this.stormParams.maxLightnings; i++) {
    var lightning = new _LightningStrike.LightningStrike(_LightningStrike.LightningStrike.copyParameters({}, this.lightningParameters));
    var mesh = new _threeModule.Mesh(lightning, this.lightningMaterial);
    this.deadLightningsMeshes.push(mesh);
  }
};

exports.LightningStorm = LightningStorm;
LightningStorm.prototype = Object.create(_threeModule.Object3D.prototype);
...
LightningStorm.prototype.update = function (time) {
  ...
  if (time >= this.nextLightningTime) {
    ...
    if (lightningMesh) {
      ...
      lightningParams1.deathTime = time + _threeModule.Math.lerp(this.stormParams.lightningMinDuration, this.stormParams.lightningMaxDuration, Math.random());
...
LightningStorm.prototype.getNextLightningTime = function (currentTime) {
  return currentTime + _threeModule.Math.lerp(this.stormParams.lightningMinPeriod, this.stormParams.lightningMaxPeriod, Math.random()) / (this.stormParams.maxLightnings + 1);
};

LightningStorm.prototype.copy = function (source) {
  _threeModule.Object3D.prototype.copy.call(this, source);
...
_LightningStrike.LightningStrike.prototype.clone = function () {
