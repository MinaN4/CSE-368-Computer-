...
exports.ColorMapKeywords = exports.Lut = void 0;
    ...
    for (var i = 0; i <= 1; i += step) {
      for (var j = 0; j < this.map.length - 1; j++) {
        if (i >= this.map[j][0] && i < this.map[j + 1][0]) {
          ...
          var minColor = new _threeModule.Color(this.map[j][1]);
          var maxColor = new _threeModule.Color(this.map[j + 1][1]);
    ...
    for (var i = 1; i >= 0; i -= step) {
      for (var j = this.map.length - 1; j >= 0; j--) {
        if (i < this.map[j][0] && i >= this.map[j - 1][0]) {
          ...
          var minColor = new _threeModule.Color(this.map[j - 1][1]);
          var maxColor = new _threeModule.Color(this.map[j][1]);
          var color = minColor.lerp(maxColor, (i - min) / (max - min));
