...
ColladaExporter.prototype = {
  ...
  parse: function parse(object, onDone, options) {
    ...
    function getTransform(o) {
      ...
      o.updateMatrix();
      transMat = transMat || new _threeModule.Matrix4();
    ...
    function processGeometry(g) {
      ...
      if (!info) {
        ...
        if (bufferGeometry instanceof _threeModule.Geometry) {
          bufferGeometry = new _threeModule.BufferGeometry().fromGeometry(bufferGeometry);
        }
    ...
    function processMaterial(m) {
      ...
      if (matid == null) {
        ...
        if (m instanceof _threeModule.MeshLambertMaterial) {
          type = 'lambert';
        } else if (m instanceof _threeModule.MeshBasicMaterial) {
          type = 'constant';
        ...
        var emissive = m.emissive ? m.emissive : new _threeModule.Color(0, 0, 0);
        var diffuse = m.color ? m.color : new _threeModule.Color(0, 0, 0);
        var specular = m.specular ? m.specular : new _threeModule.Color(1, 1, 1);
        ...
        var effectnode = "<effect id=\"".concat(matid, "-effect\">") + '<profile_COMMON>' + (m.map ? '<newparam sid="diffuse-surface"><surface type="2D">' + "<init_from>".concat(processTexture(m.map), "</init_from>") + '</surface></newparam>' + '<newparam sid="diffuse-sampler"><sampler2D><source>diffuse-surface</source></sampler2D></newparam>' : '') + (m.specularMap ? '<newparam sid="specular-surface"><surface type="2D">' + "<init_from>".concat(processTexture(m.specularMap), "</init_from>") + '</surface></newparam>' + '<newparam sid="specular-sampler"><sampler2D><source>specular-surface</source></sampler2D></newparam>' : '') + (m.emissiveMap ? '<newparam sid="emissive-surface"><surface type="2D">' + "<init_from>".concat(processTexture(m.emissiveMap), "</init_from>") + '</surface></newparam>' + '<newparam sid="emissive-sampler"><sampler2D><source>emissive-surface</source></sampler2D></newparam>' : '') + techniqueNode + (m.side === _threeModule.DoubleSide ? "<extra><technique profile=\"THREEJS\"><double_sided sid=\"double_sided\" type=\"int\">1</double_sided></technique></extra>" : '') + '</profile_COMMON>' + '</effect>';
    ...
    function processObject(o) {
      ...
      if (o instanceof _threeModule.Mesh && o.geometry != null) {
        ...
        var mat = o.material || new _threeModule.MeshBasicMaterial();
