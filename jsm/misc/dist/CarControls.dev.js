...
var CarControls = function () {
  ...
  var frontLeftWheel = new _threeModule.Group();
  var frontRightWheel = new _threeModule.Group();
  ...
  function CarControls(maxSpeed, acceleration, brakePower, turningRadius, keys) {
    ...
    maxSpeedReverse = -this.maxSpeed * 0.25; // m/s

    this.acceleration = acceleration || 10;
    accelerationReverse = this.acceleration * 0.5; // metres

    this.turningRadius = turningRadius || 6; // m/s

    deceleration = this.acceleration * 2; // multiplied with deceleration, so breaking deceleration = ( acceleration * 2 * brakePower ) m/s

    this.brakePower = brakePower || 10; // exposed so that a user can use this for various effect, e.g blur

    this.speed = 0; // keys used to control car - by default the arrow keys and space to brake

    controlKeys = keys || controlKeys; // local axes of rotation - these are likely to vary between models
  ...
  CarControls.prototype = {
    ...
    update: function update(delta) {
      ...
      if (controls.moveForward) {
        this.speed = _threeModule.Math.clamp(this.speed + delta * this.acceleration, maxSpeedReverse, this.maxSpeed);
        acceleration = _threeModule.Math.clamp(acceleration + delta, -1, 1);
      }

      if (controls.moveBackward) {
        this.speed = _threeModule.Math.clamp(this.speed - delta * accelerationReverse, maxSpeedReverse, this.maxSpeed);
        acceleration = _threeModule.Math.clamp(acceleration - delta, -1, 1);
      }

      if (controls.moveLeft) {
        wheelOrientation = _threeModule.Math.clamp(wheelOrientation + delta * steeringWheelSpeed, -maxSteeringRotation, maxSteeringRotation);
      }
      ...
      if (!(controls.moveForward || controls.moveBackward)) {
        if (this.speed > 0) {
          ...
          this.speed = _threeModule.Math.clamp(this.speed - k * delta * deceleration * brakingDeceleration, 0, this.maxSpeed);
          acceleration = _threeModule.Math.clamp(acceleration - k * delta, 0, 1);
        } else {
          var k = exponentialEaseOut(this.speed / maxSpeedReverse);
          this.speed = _threeModule.Math.clamp(this.speed + k * delta * accelerationReverse * brakingDeceleration, maxSpeedReverse, 0);
          acceleration = _threeModule.Math.clamp(acceleration + k * delta, -1, 0);
        }
      } // steering decay
      ...
      if (!(controls.moveLeft || controls.moveRight)) {
        if (wheelOrientation > 0) {
          wheelOrientation = _threeModule.Math.clamp(wheelOrientation - delta * steeringWheelSpeed, 0, maxSteeringRotation);
        } else {
          wheelOrientation = _threeModule.Math.clamp(wheelOrientation + delta * steeringWheelSpeed, -maxSteeringRotation, 0);
        }
      }

      var forwardDelta = -this.speed * delta;
      carOrientation -= forwardDelta * this.turningRadius * 0.02 * wheelOrientation; // movement of car

      root.position.x += Math.sin(carOrientation) * forwardDelta * length;
      root.position.z += Math.cos(carOrientation) * forwardDelta * length; // angle of car

      root.rotation.y = carOrientation; // wheels rolling
      ...
      backRightWheel.rotation[this.wheelRotationAxis] -= wheelDelta; // rotation while steering
    ...
    setModel: function setModel(model, elemNames) {
    ...
    setupWheels: function setupWheels() {
    ...
    computeDimensions: function computeDimensions() {
      var bb = new _threeModule.Box3().setFromObject(frontLeftWheelRoot);
      var size = new _threeModule.Vector3();
      bb.getSize(size);
