...
var _MorphBlendMesh = require("../misc/MorphBlendMesh.js");
...
var MD2CharacterComplex = function MD2CharacterComplex() {
  ...
  this.root = new _threeModule.Object3D();
  this.meshBody = null;
  ...
  this.loadParts = function (config) {
    ...
    loader.load(config.baseUrl + config.body, function (geo) {
      var boundingBox = new _threeModule.Box3();
  ...
  this.updateMovementModel = function (delta) {
    ...
    if (controls.moveForward) this.speed = _threeModule.Math.clamp(this.speed + delta * this.frontAcceleration, this.maxReverseSpeed, this.maxSpeed);
    if (controls.moveBackward) this.speed = _threeModule.Math.clamp(this.speed - delta * this.backAcceleration, this.maxReverseSpeed, this.maxSpeed); // orientation based on controls
    ...
    if (controls.moveLeft) {
      ...
      this.speed = _threeModule.Math.clamp(this.speed + dir * delta * this.frontAcceleration, this.maxReverseSpeed, this.maxSpeed);
    }
    ...
    if (!(controls.moveForward || controls.moveBackward)) {
      if (this.speed > 0) {
        ...
        this.speed = _threeModule.Math.clamp(this.speed - k * delta * this.frontDecceleration, 0, this.maxSpeed);
      } else {
        var k = exponentialEaseOut(this.speed / this.maxReverseSpeed);
        this.speed = _threeModule.Math.clamp(this.speed + k * delta * this.backAcceleration, this.maxReverseSpeed, 0);
      }
    } // displacement
  ...
  function loadTextures(baseUrl, textureUrls) {
    var textureLoader = new _threeModule.TextureLoader();
    var textures = [];

    for (var i = 0; i < textureUrls.length; i++) {
      textures[i] = textureLoader.load(baseUrl + textureUrls[i], checkLoadingComplete);
      textures[i].mapping = _threeModule.UVMapping;
  ...
  function createPart(geometry, skinMap) {
    var materialWireframe = new _threeModule.MeshLambertMaterial({
      color: 0xffaa00,
      wireframe: true,
      morphTargets: true,
      morphNormals: true
    });
    var materialTexture = new _threeModule.MeshLambertMaterial({
      color: 0xffffff,
      wireframe: false,
      map: skinMap,
      morphTargets: true,
