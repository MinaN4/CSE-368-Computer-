...
var DecalGeometry = function DecalGeometry(mesh, position, orientation, size) {
  ...
  var plane = new _threeModule.Vector3(); // this matrix represents the transformation of the decal projector

  var projectorMatrix = new _threeModule.Matrix4();
  projectorMatrix.makeRotationFromEuler(orientation);
  projectorMatrix.setPosition(position);
  var projectorMatrixInverse = new _threeModule.Matrix4().getInverse(projectorMatrix); // generate buffers
  ...
  function generate() {
    ...
    var decalVertices = [];
    var vertex = new _threeModule.Vector3();
    var normal = new _threeModule.Vector3(); // handle different geometry types
  ...
  function clip(v0, v1, p, s) {
    ...
    var s0 = d0 / (d0 - d1);
    var v = new DecalVertex(new _threeModule.Vector3(v0.position.x + s0 * (v1.position.x - v0.position.x), v0.position.y + s0 * (v1.position.y - v0.position.y), v0.position.z + s0 * (v1.position.z - v0.position.z)), new _threeModule.Vector3(v0.normal.x + s0 * (v1.normal.x - v0.normal.x), v0.normal.y + s0 * (v1.normal.y - v0.normal.y), v0.normal.z + s0 * (v1.normal.z - v0.normal.z))); // need to clip more values (texture coordinates)? do it this way:
...
DecalGeometry.prototype = Object.create(_threeModule.BufferGeometry.prototype);
