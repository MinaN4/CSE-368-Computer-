...
VTKLoader.prototype = Object.assign(Object.create(_threeModule.Loader.prototype), {
  ...
  parse: function parse(data) {
    function parseASCII(data) {
      ...
      var normals = [];
      var result; // pattern for reading vertices, 3 floats or integers
      ...
      if (colors.length !== indices.length) {
        ...
        if (colors.length === positions.length) {
          geometry.setAttribute('color', new _threeModule.Float32BufferAttribute(colors, 3));
        }
      } else {
        // cell
        ...
        if (colors.length === numTriangles * 3) {
          ...
          geometry.setAttribute('color', new _threeModule.Float32BufferAttribute(newColors, 3));
        }
      }
    ...
    function parseBinary(data) {
      ...
      var geometry = new _threeModule.BufferGeometry();
      geometry.setIndex(new _threeModule.BufferAttribute(indices, 1));
      geometry.setAttribute('position', new _threeModule.BufferAttribute(points, 3));

      if (normals.length === points.length) {
        geometry.setAttribute('normal', new _threeModule.BufferAttribute(normals, 3));
      }
      ...
      function parseDataArray(ele, compressed) {
        ...
        if (ele.attributes.format === 'binary' && compressed) {
          ...
          for (var i = 0; i < dataOffsets.length - 1; i++) {
            var inflate = new _inflateModuleMin.Zlib.Inflate(byteData.slice(dataOffsets[i], dataOffsets[i + 1]), {
              resize: true,
              verify: true
    ...
    var meta = _threeModule.LoaderUtils.decodeText(new Uint8Array(data, 0, 250)).split('\n');
