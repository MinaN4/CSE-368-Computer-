...
function printBuffer(buffer, from, to) {
  console.log(_threeModule.LoaderUtils.decodeText(new Uint8Array(buffer, from, to)));
}
...
LWOLoader.prototype = Object.assign(Object.create(_threeModule.Loader.prototype), {
  constructor: LWOLoader,
  load: function load(url, onLoad, onProgress, onError) {
    var self = this;
    var path = self.path === '' ? extractParentUrl(url, 'Objects') : self.path; // give the mesh a default name based on the filename
    ...
    loader.load(url, function (buffer) {
      ...
      onLoad(self.parse(buffer, path, modelName)); // console.timeEnd( 'Total parsing: ' );
    }, onProgress, onError);
  },
...
LWOTreeParser.prototype = {
  ...
  parseMesh: function parseMesh(geometry, layer) {
    ...
    if (layer.geometry.type === 'points') mesh = new _threeModule.Points(geometry, materials);else if (layer.geometry.type === 'lines') mesh = new _threeModule.LineSegments(geometry, materials);else mesh = new _threeModule.Mesh(geometry, materials);
    ...
    if (type === 'points' || type === 'lines') {
      materials.forEach(function (mat, i) {
        ...
        if (type === 'points') {
          ...
          materials[i] = new _threeModule.PointsMaterial(spec);
        } else if (type === 'lines') {
          materials[i] = new _threeModule.LineBasicMaterial(spec);
        }
      });
...
MaterialParser.prototype = {
  ...
  /*, textures*/
    ...
    params = Object.assign(params, attributes);
    return new _threeModule.MeshPhongMaterial(params);
  },
  // Note: converting from left to right handed coords by switching x -> -x in vertices, and
  // then switching mat FrontSide -> BackSide
  // NB: this means that FrontSide and BackSide have been switched!
  getSide: function getSide(attributes) {
    if (!attributes.side) return _threeModule.BackSide;

    switch (attributes.side) {
      case 0:
      case 1:
        return _threeModule.BackSide;

      case 2:
        return _threeModule.FrontSide;

      case 3:
        return _threeModule.DoubleSide;
    }
  },
    ...
    for (var name in textureNodes) {
      ...
      switch (name) {
        ...
        case 'Normal':
          ...
          if (node.amplitude !== undefined) maps.normalScale = new _threeModule.Vector2(node.amplitude, node.amplitude);
          break;
  ...
  parseAttributes: function parseAttributes(attributes, maps) {
    ...
    if (attributes.Color && !maps.map) {
      params.color = new _threeModule.Color().fromArray(attributes.Color.value);
    } else params.color = new _threeModule.Color();
  ...
  parseStandardAttributes: function parseStandardAttributes(params, attributes, maps) {
    if (attributes.Luminous) {
      ...
      if (attributes['Luminous Color'] && !maps.emissive) {
        params.emissive = new _threeModule.Color().fromArray(attributes['Luminous Color'].value);
      } else {
        params.emissive = new _threeModule.Color(0x808080);
      }
    }
  ...
  parsePhongAttributes: function parsePhongAttributes(params, attributes, maps) {
    ...
    if (attributes.Reflection) {
      ...
      params.combine = _threeModule.AddOperation;
    }
    ...
    if (!attributes.Roughness && attributes.Specular && !maps.specularMap) {
      if (attributes['Color Highlight']) {
        params.specular = new _threeModule.Color().setScalar(attributes.Specular.value).lerp(params.color.clone().multiplyScalar(attributes.Specular.value), attributes['Color Highlight'].value);
      } else {
        params.specular = new _threeModule.Color().setScalar(attributes.Specular.value);
      }
    }
  ...
  parseEnvMap: function parseEnvMap(connections, maps, attributes) {
    if (connections.envMap) {
      ...
      if (attributes.transparent && attributes.opacity < 0.999) {
        envMap.mapping = _threeModule.EquirectangularRefractionMapping; // Reflectivity and refraction mapping don't work well together in Phong materials
      ...
      } else envMap.mapping = _threeModule.EquirectangularReflectionMapping;
  ...
  getWrappingType: function getWrappingType(num) {
    switch (num) {
      case 0:
        ...
        return _threeModule.ClampToEdgeWrapping;

      case 1:
        return _threeModule.RepeatWrapping;

      case 2:
        return _threeModule.MirroredRepeatWrapping;
  ...
  getMaterialType: function getMaterialType(nodeData) {
    if (nodeData.Clearcoat && nodeData.Clearcoat.value > 0) return _threeModule.MeshPhysicalMaterial;
    if (nodeData.Roughness) return _threeModule.MeshStandardMaterial;
    return _threeModule.MeshPhongMaterial;
  }
};
...
GeometryParser.prototype = {
  ...
  parse: function parse(geoData, layer) {
    ...
    geometry.setAttribute('position', new _threeModule.Float32BufferAttribute(geoData.points, 3));
  ...
  parseUVs: function parseUVs(geometry, layer) {
    ...
    geometry.setAttribute('uv', new _threeModule.Float32BufferAttribute(remappedUVs, 2));
  },
  parseMorphTargets: function parseMorphTargets(geometry, layer) {
    ...
    for (var name in layer.morphTargets) {
      ...
      geometry.morphAttributes.position[num] = new _threeModule.Float32BufferAttribute(remappedPoints, 3);
