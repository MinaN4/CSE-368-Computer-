...
BasisTextureLoader.prototype = Object.assign(Object.create(_threeModule.Loader.prototype), {
  ...
  load: function load(url, onLoad, onProgress, onError) {
    ...
    loader.load(url, function (buffer) {
      _this._createTexture(buffer).then(onLoad)["catch"](onError);
    }, onProgress, onError);
  },
   ...
   */
  _createTexture: function _createTexture(buffer) {
    var _this2 = this;

    var worker;
    var taskID;

    var texturePending = this._getWorker().then(function (_worker) {
      worker = _worker;
      ...
      switch (format) {
        case BasisTextureLoader.BASIS_FORMAT.cTFASTC_4x4:
          texture = new _threeModule.CompressedTexture(mipmaps, width, height, _threeModule.RGBA_ASTC_4x4_Format);
          break;

        case BasisTextureLoader.BASIS_FORMAT.cTFBC1:
        case BasisTextureLoader.BASIS_FORMAT.cTFBC3:
          texture = new _threeModule.CompressedTexture(mipmaps, width, height, BasisTextureLoader.DXT_FORMAT_MAP[config.format], _threeModule.UnsignedByteType);
          break;

        case BasisTextureLoader.BASIS_FORMAT.cTFETC1:
          texture = new _threeModule.CompressedTexture(mipmaps, width, height, _threeModule.RGB_ETC1_Format);
          break;

        case BasisTextureLoader.BASIS_FORMAT.cTFPVRTC1_4_RGB:
          texture = new _threeModule.CompressedTexture(mipmaps, width, height, _threeModule.RGB_PVRTC_4BPPV1_Format);
          break;

        case BasisTextureLoader.BASIS_FORMAT.cTFPVRTC1_4_RGBA:
          texture = new _threeModule.CompressedTexture(mipmaps, width, height, _threeModule.RGBA_PVRTC_4BPPV1_Format);
          break;
      ...
      texture.minFilter = mipmaps.length === 1 ? _threeModule.LinearFilter : _threeModule.LinearMipmapLinearFilter;
    ...
    texturePending["finally"](function () {
      if (worker && taskID) {
        worker._taskLoad -= worker._taskCosts[taskID];
  ...
  _initTranscoder: function _initTranscoder() {
    ...
    if (!this.transcoderBinary) {
      ...
      var jsLoader = new _threeModule.FileLoader(this.manager);
      jsLoader.setPath(this.transcoderPath);
      var jsContent = new Promise(function (resolve, reject) {
        jsLoader.load('basis_transcoder.js', resolve, undefined, reject);
      }); // Load transcoder WASM binary.

      var binaryLoader = new _threeModule.FileLoader(this.manager);
      ...
      this.transcoderPending = Promise.all([jsContent, binaryContent]).then(function (_ref) {
        ...
        var body = ['/* basis_transcoder.js */', jsContent, '/* worker */', fn.substring(fn.indexOf('{') + 1, fn.lastIndexOf('}'))].join('\n');
  ...
  _getWorker: function _getWorker() {
    var _this4 = this;
    ...
    return this._initTranscoder().then(function () {
      ...
      return _this4.workerPool[_this4.workerPool.length - 1];
    });
  },
  ...
  function transcode(buffer) {
    ...
    if (!hasAlpha) {
      switch (config.format) {
        case 9:
          // Hardcoded: BasisTextureLoader.BASIS_FORMAT.cTFPVRTC1_4_RGBA
