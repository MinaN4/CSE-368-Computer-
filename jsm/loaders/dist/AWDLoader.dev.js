...
var AWDLoader = function () {
  ...
  var AWDLoader = function AWDLoader(manager) {
    ...
    this.trunk = new _threeModule.Object3D();
  ...
  AWDLoader.prototype = Object.assign(Object.create(_threeModule.Loader.prototype), {
    ...
    parseContainer: function parseContainer() {
      var parent,
          ctr = new _threeModule.Object3D(),
          par_id = this.readU32(),
    ...
    parseMeshInstance: function parseMeshInstance() {
      ...
      meshes = []; // TODO : BufferGeometry don't support "geometryGroups" for now.
      // so we create sub meshes for each groups

      if (meshLen > 1) {
        mesh = new _threeModule.Object3D();

        for (i = 0; i < meshLen; i++) {
          var sm = new _threeModule.Mesh(geometries[i]);
          meshes.push(sm);
          mesh.add(sm);
        }
      } else {
        mesh = new _threeModule.Mesh(geometries[0]);
        meshes.push(mesh);
      }
    ...
    parseMaterial: function parseMaterial() {
      var name, type, props, mat, attributes, num_methods, methods_parsed;
      name = this.readUTF();
      type = this.readU8();
      num_methods = this.readU8(); //log( "AWDLoader parseMaterial ",name )
      ...
      mat = new _threeModule.MeshPhongMaterial();
    ...
    parseTexture: function parseTexture() {
      ...
      } else {} // embed texture not supported
      // Ignore for now
    ...
    loadTexture: function loadTexture(url) {
      var tex = new _threeModule.Texture();
      var loader = new _threeModule.ImageLoader(this.manager);
    ...
    parseSkeleton: function parseSkeleton() {
      // Array<Bone>
      //
      this.readUTF();
      ...
      while (joints_parsed < num_joints) {
        var joint, ibp; // Ignore joint id

        this.readU16();
        joint = new _threeModule.Bone();
        joint.parent = this.readU16() - 1; // 0=null in AWD

        joint.name = this.readUTF();
        ibp = this.parseMatrix4();
        joint.skinMatrix = ibp; // Ignore joint props/attributes for now
    ...
    parseSkeletonPose: function parseSkeletonPose() {
      var name = this.readUTF();
      var num_joints = this.readU16();
      this.parseProperties(null); // debug( 'parse Skeleton Pose. joints : ' + num_joints);
      ...
      while (joints_parsed < num_joints) {
        ...
        if (has_transform === 1) {
          ...
          mtx_data = new _threeModule.Matrix4();
        }
    ...
    parseSkeletonAnimation: function parseSkeletonAnimation() {
      var frame_dur;
      var pose_addr;
      var pose;
      var name = this.readUTF();
      ...
      this.parseProperties(null);
      var frames_parsed = 0; // debug( 'parse Skeleton Animation. frames : ' + num_frames);

      while (frames_parsed < num_frames) {
        pose_addr = this.readU32();
        frame_dur = this.readU16();
        pose = this._blocks[pose_addr].data; // debug( 'pose address ',pose[2].elements[12],pose[2].elements[13],pose[2].elements[14] );
    ...
    parseVertexAnimationSet: function parseVertexAnimationSet() {
      var poseBlockAdress,
          name = this.readUTF(),
    ...
    parseAnimatorSet: function parseAnimatorSet() {
      ...
      } else if (type == 2) {// debug( "vertex Anim???");
      }
    ...
    parseMeshData: function parseMeshData() {
      var name = this.readUTF(),
          num_subs = this.readU16(),
          geom,
          subs_parsed = 0,
          buffer,
          geometries = []; // Ignore for now
      ...
      while (subs_parsed < num_subs) {
        ...
        geom = new _threeModule.BufferGeometry();
        geom.name = name;
        geometries.push(geom);
        sm_len = this.readU32();
        sm_end = this._ptr + sm_len; // Ignore for now
        ...
        while (this._ptr < sm_end) {
          ...
          if (str_type === 1) {
            ...
            attrib = new _threeModule.BufferAttribute(buffer, 3);
            ...
            attrib = new _threeModule.BufferAttribute(buffer, 1);
            ...
            attrib = new _threeModule.BufferAttribute(buffer, 2);
      ...
      this.parseUserAttributes(); //finalizeAsset(geom, name);

      return geometries;
    },
    parseMeshPoseAnimation: function parseMeshPoseAnimation(poseOnly) {
      var num_frames = 1,
          num_submeshes,
          frames_parsed,
          subMeshParsed,
      ...
      num_Streams = this.readU16(); // debug("VA num_frames : ", num_frames );
    ...
    parseMatrix4: function parseMatrix4() {
      var mtx = new _threeModule.Matrix4();
      var e = mtx.elements;
      e[0] = this.readF32();
      e[1] = this.readF32();
      e[2] = this.readF32();
      ...
      e[4] = this.readF32();
      e[5] = this.readF32();
      e[6] = this.readF32(); //e[7] = this.readF32();

      e[7] = 0.0;
      e[8] = this.readF32();
      e[9] = this.readF32();
      e[10] = this.readF32(); //e[11] = this.readF32();

      e[11] = 0.0;
      e[12] = -this.readF32();
      e[13] = this.readF32();
      e[14] = this.readF32(); //e[15] = this.readF32();
