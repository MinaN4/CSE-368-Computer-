...
PCDLoader.prototype = Object.assign(Object.create(_threeModule.Loader.prototype), {
  ...
  parse: function parse(data, url) {
    ...
    var textData = _threeModule.LoaderUtils.decodeText(new Uint8Array(data)); // parse header (always ascii format)
    ...
    if (position.length > 0) geometry.setAttribute('position', new _threeModule.Float32BufferAttribute(position, 3));
    if (normal.length > 0) geometry.setAttribute('normal', new _threeModule.Float32BufferAttribute(normal, 3));
    if (color.length > 0) geometry.setAttribute('color', new _threeModule.Float32BufferAttribute(color, 3));
    geometry.computeBoundingSphere(); // build material

    var material = new _threeModule.PointsMaterial({
      size: 0.005
    });
    ...
    var mesh = new _threeModule.Points(geometry, material);
