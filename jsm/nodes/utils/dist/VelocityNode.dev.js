...
function VelocityNode(target, params) {
  ...
  this.params = {};
  this.velocity = new _threeModule.Vector3();
...
VelocityNode.prototype.setParams = function (params) {
  ...
  switch (this.params.type) {
    case "elastic":
      this.moment = new _threeModule.Vector3();
      this.speed = new _threeModule.Vector3();
      this.springVelocity = new _threeModule.Vector3();
      this.lastVelocity = new _threeModule.Vector3();
      break;
  }
};
...
VelocityNode.prototype.setTarget = function (target) {
  ...
  if (target) {
    this.position = target.getWorldPosition(this.position || new _threeModule.Vector3());
...
{
  if (this.target) {
    this.position = this.target.getWorldPosition(this.position || new _threeModule.Vector3());
