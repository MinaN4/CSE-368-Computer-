...
ColorSpaceNode.Nodes = function () {
  ...
  var LinearToRGBD = new _FunctionNode.FunctionNode(["vec3 LinearToRGBD( in vec4 value, in float maxRange ) {", "	float maxRGB = max( value.x, max( value.g, value.b ) );", "	float D      = max( maxRange / maxRGB, 1.0 );", "	D            = min( floor( D ) / 255.0, 1.0 );", "	return vec4( value.rgb * ( D * ( 255.0 / maxRange ) ), D );", "}"].join("\n")); // LogLuv reference: http://graphicrants.blogspot.ca/2009/04/rgbm-color-encoding.html
...
ColorSpaceNode.getEncodingComponents = function (encoding) {
  switch (encoding) {
    case _threeModule.LinearEncoding:
      return ['Linear'];

    case _threeModule.sRGBEncoding:
      return ['sRGB'];

    case _threeModule.RGBEEncoding:
      return ['RGBE'];

    case _threeModule.RGBM7Encoding:
      return ['RGBM', new _FloatNode.FloatNode(7.0).setReadonly(true)];

    case _threeModule.RGBM16Encoding:
      return ['RGBM', new _FloatNode.FloatNode(16.0).setReadonly(true)];

    case _threeModule.RGBDEncoding:
      return ['RGBD', new _FloatNode.FloatNode(256.0).setReadonly(true)];

    case _threeModule.GammaEncoding:
