...
function StandardNode() {
  ...
  this.roughness = new _FloatNode.FloatNode(0.5);
  this.metalness = new _FloatNode.FloatNode(0.5);
...
StandardNode.prototype.build = function (builder) {
  ...
  if (builder.isShader('vertex')) {
    ...
    builder.mergeUniform(_threeModule.UniformsUtils.merge([_threeModule.UniformsLib.fog, _threeModule.UniformsLib.lights]));

    if (_threeModule.UniformsLib.LTC_1) {
    ...
    output.push('material.diffuseColor = ' + (light ? 'vec3( 1.0 )' : 'diffuseColor * (1.0 - metalnessFactor)') + ';', 'material.specularRoughness = clamp( roughnessFactor, 0.04, 1.0 );');
    ...
    if (clearcoatRoughness) {
      output.push(clearcoatRoughness.code, 'material.clearcoatRoughness = clamp( ' + clearcoatRoughness.result + ', 0.04, 1.0 );');
    } else if (useClearcoat) {
