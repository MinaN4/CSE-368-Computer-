...
function WebGLTextures(_gl, extensions, state, properties, capabilities, utils, info) {
  ...
  function resizeImage(image, needsPowerOfTwo, needsNewCanvas, maxSize) {
    ...
    if (scale < 1 || needsPowerOfTwo === true) {
      ...
      if (typeof HTMLImageElement !== 'undefined' && image instanceof HTMLImageElement || typeof HTMLCanvasElement !== 'undefined' && image instanceof HTMLCanvasElement || typeof ImageBitmap !== 'undefined' && image instanceof ImageBitmap) {
        var floor = needsPowerOfTwo ? _Math2._Math.floorPowerOfTwo : Math.floor;
  ...
  function isPowerOfTwo(image) {
    return _Math2._Math.isPowerOfTwo(image.width) && _Math2._Math.isPowerOfTwo(image.height);
  }
      ...
      if (isCompressed) {
        for (var i = 0; i < 6; i++) {
          ...
          for (var j = 0; j < mipmaps.length; j++) {
            ...
            if (texture.format !== _constants.RGBAFormat && texture.format !== _constants.RGBFormat) {
              if (glFormat !== null) {
  ...
  var wrappingToGL = (_wrappingToGL = {}, _defineProperty(_wrappingToGL, _constants.RepeatWrapping, _gl.REPEAT), _defineProperty(_wrappingToGL, _constants.ClampToEdgeWrapping, _gl.CLAMP_TO_EDGE), _defineProperty(_wrappingToGL, _constants.MirroredRepeatWrapping, _gl.MIRRORED_REPEAT), _wrappingToGL);
  var filterToGL = (_filterToGL = {}, _defineProperty(_filterToGL, _constants.NearestFilter, _gl.NEAREST), _defineProperty(_filterToGL, _constants.NearestMipmapNearestFilter, _gl.NEAREST_MIPMAP_NEAREST), _defineProperty(_filterToGL, _constants.NearestMipmapLinearFilter, _gl.NEAREST_MIPMAP_LINEAR), _defineProperty(_filterToGL, _constants.LinearFilter, _gl.LINEAR), _defineProperty(_filterToGL, _constants.LinearMipmapNearestFilter, _gl.LINEAR_MIPMAP_NEAREST), _defineProperty(_filterToGL, _constants.LinearMipmapLinearFilter, _gl.LINEAR_MIPMAP_LINEAR), _filterToGL);
  ...
  function setTextureParameters(textureType, texture, supportsMips) {
    ...
    if (extension) {
      if (texture.type === _constants.FloatType && extensions.get('OES_texture_float_linear') === null) return;
      if (texture.type === _constants.HalfFloatType && (isWebGL2 || extensions.get('OES_texture_half_float_linear')) === null) return;
  ...
  function uploadTexture(textureProperties, texture, slot) {
    ...
    if (texture.isDepthTexture) {
      ...
      if (texture.format === _constants.DepthStencilFormat) {
