...
function getEncodingComponents(encoding) {
  switch (encoding) {
    ...
    case _constants.RGBEEncoding:
      return ['RGBE', '( value )'];

    case _constants.RGBM7Encoding:
      return ['RGBM', '( value, 7.0 )'];

    case _constants.RGBM16Encoding:
      return ['RGBM', '( value, 16.0 )'];

    case _constants.RGBDEncoding:
      return ['RGBD', '( value, 256.0 )'];

    case _constants.GammaEncoding:
      return ['Gamma', '( value, float( GAMMA_FACTOR ) )'];

    case _constants.LogLuvEncoding:
      return ['LogLuv', '( value )'];
...
function getToneMappingFunction(functionName, toneMapping) {
  ...
  switch (toneMapping) {
    ...
    case _constants.Uncharted2ToneMapping:
...
function generateEnvMapTypeDefine(parameters) {
  ...
  if (parameters.envMap) {
    switch (parameters.envMapMode) {
      ...
      case _constants.CubeUVRefractionMapping:
        envMapTypeDefine = 'ENVMAP_TYPE_CUBE_UV';
        break;

      case _constants.EquirectangularReflectionMapping:
      case _constants.EquirectangularRefractionMapping:
        envMapTypeDefine = 'ENVMAP_TYPE_EQUIREC';
        break;

      case _constants.SphericalReflectionMapping:
