...
var _WebGLMultiview = require("./webgl/WebGLMultiview.js");

var _WebVRManager = require("./webvr/WebVRManager.js");

var _WebXRManager = require("./webvr/WebXRManager.js");
...
function WebGLRenderer(parameters) {
  ...
  this.toneMapping = _constants.LinearToneMapping;
  ...
  _currentGeometryProgram = {
    ...
    wireframe: false
      ...
      _currentViewport = new _Vector3.Vector4(),
      _currentScissor = new _Vector3.Vector4(),
      _currentScissorTest = null,
      //
  _width = _canvas.width,
      _height = _canvas.height,
      _pixelRatio = 1,
      _viewport = new _Vector3.Vector4(0, 0, _width, _height),
      _scissor = new _Vector3.Vector4(0, 0, _width, _height),
      _scissorTest = false,
      // frustum
  _frustum = new _Frustum.Frustum(),
      // clipping
  _clipping = new _WebGLClipping.WebGLClipping(),
      _clippingEnabled = false,
      _localClippingEnabled = false,
      // camera matrices cache
  _projScreenMatrix = new _Matrix.Matrix4(),
      _vector3 = new _Vector2.Vector3();
  ...
  function initGLContext() {
    ...
    geometries = new _WebGLGeometries.WebGLGeometries(_gl, attributes, info);
    objects = new _WebGLObjects.WebGLObjects(_gl, geometries, attributes, info);
    morphtargets = new _WebGLMorphtargets.WebGLMorphtargets(_gl);
    programCache = new _WebGLPrograms.WebGLPrograms(_this, extensions, capabilities);
    renderLists = new _WebGLRenderLists.WebGLRenderLists();
    renderStates = new _WebGLRenderStates.WebGLRenderStates();
    background = new _WebGLBackground.WebGLBackground(_this, state, objects, _premultipliedAlpha);
  ...
  var vr = typeof navigator !== 'undefined' && 'xr' in navigator ? new _WebXRManager.WebXRManager(_this, _gl) : new _WebVRManager.WebVRManager(_this);
  this.vr = vr; // Multiview

  var multiview = new _WebGLMultiview.WebGLMultiview(_this, _gl); // shadow map

  var shadowMap = new _WebGLShadowMap.WebGLShadowMap(_this, objects, capabilities.maxTextureSize);
    ...
    if (object.isMesh) {
      if (material.wireframe === true) {
        ...
        switch (object.drawMode) {
          case _constants.TrianglesDrawMode:
            renderer.setMode(_gl.TRIANGLES);
            break;

          case _constants.TriangleStripDrawMode:
            renderer.setMode(_gl.TRIANGLE_STRIP);
            break;

          case _constants.TriangleFanDrawMode:
  ...
  function initMaterial(material, fog, object) {
    ...
    if (programChange) {
      if (parameters.shaderID) {
        var shader = _ShaderLib.ShaderLib[parameters.shaderID];
    ...
    var progUniforms = materialProperties.program.getUniforms(),
        uniformsList = _WebGLUniforms.WebGLUniforms.seqWithValue(progUniforms.seq, uniforms);
  ...
  function setProgram(camera, fog, material, object) {
      ...
      if (skeleton) {
        ...
        if (capabilities.floatVertexTextures) {
          if (skeleton.boneTexture === undefined) {
            ...
            size = _Math2._Math.ceilPowerOfTwo(size);
    ...
    if (refreshMaterial) {
      ...
      if (m_uniforms.ltc_1 !== undefined) m_uniforms.ltc_1.value = _UniformsLib.UniformsLib.LTC_1;
      if (m_uniforms.ltc_2 !== undefined) m_uniforms.ltc_2.value = _UniformsLib.UniformsLib.LTC_2;
  ...
  this.readRenderTargetPixels = function (renderTarget, x, y, width, height, buffer, activeCubeFaceIndex) {
    ...
    if (framebuffer) {
      ...
      try {
        ...
        if (textureType !== _constants.UnsignedByteType && utils.convert(textureType) !== _gl.getParameter(_gl.IMPLEMENTATION_COLOR_READ_TYPE) && // IE11, Edge and Chrome Mac < 52 (#9513)
        !(textureType === _constants.FloatType && (capabilities.isWebGL2 || extensions.get('OES_texture_float') || extensions.get('WEBGL_color_buffer_float'))) && // Chrome Mac >= 52 and Firefox
        !(textureType === _constants.HalfFloatType && (capabilities.isWebGL2 ? extensions.get('EXT_color_buffer_float') : extensions.get('EXT_color_buffer_half_float')))) {
