...
Object.assign(Curve.prototype, {
  ...
  computeFrenetFrames: function computeFrenetFrames(segments, closed) {
    ...
    normals[0] = new _Vector.Vector3();
    binormals[0] = new _Vector.Vector3();
    ...
    for (i = 1; i <= segments; i++) {
      ...
      if (vec.length() > Number.EPSILON) {
        vec.normalize();
        theta = Math.acos(_Math2._Math.clamp(tangents[i - 1].dot(tangents[i]), -1, 1)); // clamp for floating pt errors
    ...
    if (closed === true) {
      theta = Math.acos(_Math2._Math.clamp(normals[0].dot(normals[segments]), -1, 1));
      theta /= segments;
