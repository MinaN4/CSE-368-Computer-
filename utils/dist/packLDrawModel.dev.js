...
var materialsContent = fs.readFileSync(materialsFilePath, {
  encoding: "utf8"
});
...
var objectsContents = [];
var pathMap = {};
var listOfNotFound = []; // Parse object tree

parseObject(fileName, true); // Check if previously files not found are found now
...
function parseObject(fileName, isRoot) {
  ...
  for (var attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      objectContent = fs.readFileSync(absoluteObjectPath, {
        encoding: "utf8"
      });
      break;
    } catch (e) {
  ...
  for (var i = 0, n = lines.length; i < n; i++) {
    var line = lines[i];
    var lineLength = line.length; // Skip spaces/tabs
    ...
    if (line.startsWith('1 ')) {
      // Subobject, add it
      charIndex = 2; // Skip material, position and transform
